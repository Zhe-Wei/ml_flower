  ...
  with torch.no_grad():
    for i, (inputs, labels) in enumerate(dataloaders['val']):
      ...
      for j in range(inputs.size()[0]):
        ...
        plt.savefig(os.path.join('./logs_resnext_32/' + log_time, 'predict.jpg'))
...
def imshow(inp, title=None):
  ...
  os.mkdir('./logs_resnext_32/'+log_time)
  plt.savefig(os.path.join('./logs_resnext_32/' + log_time, 'Normalize1.png'))
  # if title is not None:
  #     plt.title(title)
  plt.pause(0.001)  # pause a bit so that plots are updated

  plt.imshow(inp1)
  plt.savefig(os.path.join('./logs_resnext_32/' + log_time, 'non-Normalize.png'))
...
def train_model(model, criterion, device, dataloaders, dataset_sizes, optimizer, scheduler, num_epochs=25):
  ...
  plt.ylabel('loss')
  plt.legend()
  plt.savefig(os.path.join('./logs_resnext_32/' + log_time, './eval_loss.png'))
...
def main():
  ...
  for lr in lr_arr:
    ...
    for batch_size in range(4, 16, 1):
